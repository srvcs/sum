use srvcs_sum::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-sum listening");
    let deps = Deps {
        add_url: cfg.add_url.clone(),
    };
